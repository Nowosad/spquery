single_dist_fun = function(x, y, dist_fun, ndim = NULL, ...) {
  if (dist_fun %in% philentropy::getDistMethods()) {
    calc_fun = philentropy::dist_one_one(
      x, y, method = dist_fun, testNA = FALSE, ...
    )
  } else if (
    requireNamespace("proxy", quietly = TRUE) &&
      dist_fun %in% names(summary(proxy::pr_DB)$names)
  ) {
    calc_fun = proxy_dist_one_one(x, y, method = dist_fun, ...)
  } else if (
    requireNamespace("dtwclust", quietly = TRUE) && dist_fun == "dtw"
  ) {
    if (is.null(ndim)) {
      stop("ndim must be supplied when dist_fun = 'dtw'")
    }
    calc_fun = dtw_multidim(x, y, ndim = ndim, ...)
  } else {
    stop("Unsupported distance measure: ", dist_fun)
  }
  return(calc_fun)
}
